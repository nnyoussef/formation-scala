...
class Empty extends TweetSet:
  ...
  override def filter(p: Tweet => Boolean): TweetSet = Empty()
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  override def filter(p: Tweet => Boolean): TweetSet =
    var res: TweetSet = Empty()

    def f: Tweet => Unit = (t: Tweet) => {
      if p(t) then res = res.incl(Tweet(t.user, t.text, t.retweets))
    }
  ...
  private def inclByTweet(x: Tweet): NonEmpty =
    if x.retweets < elem.retweets then
      NonEmpty(elem, left.incl(x), right)
    else if elem.retweets < x.retweets then
      NonEmpty(elem, left, right.incl(x))
    else
      this
  ...
  override def descendingByRetweet: TweetList =
    ...
    def computeMaxAndPush(): TweetList = {
      val max = tweetSet.mostRetweeted
      tweetSet = tweetSet.remove(max)
      if max != null then
        new Cons(max, computeMaxAndPush())
      else
        Nil
    }
  ...
  override def mostRetweeted: Tweet =
    var curr: Tweet = elem
    foreach((t: Tweet) => if curr.retweets < t.retweets then curr = t)
    curr


  override def union(that: TweetSet): TweetSet =
    var res: TweetSet = NonEmpty(elem, left, right)
    that.foreach((e: Tweet) => {
      res = res.incl(e)
    })
    res
